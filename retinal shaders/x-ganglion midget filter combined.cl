...
{{
    ...
    //night:
    /*uchar csq2 = get_some_surround_square_avg(coord, 20, 8, rgb_in);
    uchar csq1 = get_some_surround_square_avg(coord, 10, 8, rgb_in);
    //uchar c = guardGetColor(coord, rgb_in);
    char diff = max(csq1-csq2, 0);*/
    //day:
//note: dark on light bg: black line inner, white line outer. Opposite for light on dark bg.
    uchar c = guardGetColor(coord, rgb_in, edge_brightness);
    int csq3 = get_some_surround_square_avg(coord, 4, 2, rgb_in, c);//larger resolutions
    ...
    int diff = max((int)(c)-csq1,(int)0) + max(csq1-csq2, (int)0)/4 + max(csq2-csq3, (int)0)/16;
    int diff2 = max(csq1-(int)(c),(int)0) + max(csq2-csq1, (int)0)/4 + max(csq3-csq2, (int)0)/16;

    if(get_global_id(2)==0){{
        rgc_out[gindex2( outcoord )] = 127;
    }}
    rgc_out[gindex2( outcoord )] = min(max(((int)(rgc_out[gindex2( outcoord )])+(diff-diff2)*4), (int)0), 255);
